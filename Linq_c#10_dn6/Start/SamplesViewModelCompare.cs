...
{
  public class SamplesViewModelCompare : ViewModelBase
  {
    ...
    public bool SequenceEqualIntegersQuery()
    {
      ...
      value = (from l1 in list1 select l1)
                .SequenceEqual(list2);
    ...
    public List<Product> ExceptByProductSalesQuery()
    {
      ...
      list = (from prod in products select prod)
                .ExceptBy<Product,int>(
                    from s in sales select s.ProductID,
                    prod => prod.ProductID
              ).ToList();
    ...
    public List<Product> ExceptByProductSalesMethod()
    {
      ...
      list = products.ExceptBy<Product, int>(
                        sales.Select(s => s.ProductID),prod => prod.ProductID
                    ).ToList();
    ...
    public List<int> IntersectIntegersQuery()
    {
      ...
      List<int> list1 = new() { 5, 2, 3, 4, 5 };
      // Create a list of numbers
      List<int> list2 = new() { 3, 4, 5, 9 };

      // Write Query Syntax Here
      list = (from nums in list1 select nums)
    ...
    public List<int> IntersectProductSalesMethod()
    {
      ...
      list = products.Select(prod => prod.ProductID)
                .Intersect(sales.Select(s => s.ProductID)).ToList();
    ...
    public List<Product> IntersectByProductSalesMethod()
    {
      ...
      list = products.IntersectBy<Product,int>(
            sales.Select(sale => sale.ProductID),p => p.ProductID).ToList();
